...
export default function SellerEditProduct() {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_BACKEND_API_URL}/api/sellers/products/${id}`, 
      { headers: { Authorization: `Bearer ${token}` } }
    ).then(({ data }) => {
      // Provide default empty arrays for images/videos if missing
      setForm({
        ...data,
  ...
  // Handle array changes for images/videos
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    try {
      // Validate positive stock and price
      if (!form.name || !form.category ||
          !form.price || !form.stock || form.price <= 0 || form.stock <= 0) {
        setErr("Fill all required fields correctly.");
        return;
      }

      // Main image overridden by first url in images array
      const imagesFiltered = form.images.filter(Boolean);
      const payload = {
        ...form,
        stock: Number(form.stock),
        price: Number(form.price),
        image: imagesFiltered[0] || "",
        images: imagesFiltered,
        videos: form.videos.filter(Boolean),
      };

      await axios.put(
        `${import.meta.env.VITE_BACKEND_API_URL}/api/sellers/products/${id}`,
        payload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#f8f6fc] flex items-center justify-center">
        <form className="bg-white p-10 rounded-xl shadow-md w-full max-w-xl" onSubmit={handleSave}>
          <h2 className="mb-6 text-2xl font-bold text-[#440077]">Edit Product</h2>
          ...
          {/* Old image field (disabled as main image set from images) */}
