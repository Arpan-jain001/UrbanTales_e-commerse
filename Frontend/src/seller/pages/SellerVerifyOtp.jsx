...
export default function SellerVerifyOtp() {
  ...
  const [timer, setTimer] = useState(2 * 60); // 2 minutes in seconds (120 seconds)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${apiBase}/api/sellers/auth/verify-otp`,
        {
          method: "POST",
      ...
      if (res.ok) {
        ...
        setTimeout(
          () =>
            navigate(
              `/seller/reset-password?email=${encodeURIComponent(
                email
              )}&otp=${otp}`
            ),
          500
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimer(2 * 60); // Reset to 2 minutes
        setOTP("");
      } else {
        ...
        >
          ...
          <div className="text-center">
            {timer > 0 ? (
              <div className="flex items-center justify-center space-x-2">
                ...
                <span className={`text-sm font-semibold ${timer < 30 ? 'text-red-500' : 'text-gray-600'}`}>
          ...
          <div className="text-center">
            {timer > 0 ? (
              ...
              <button
                type="button"
                className="text-sm bg-gradient-to-r from-[#070A52] to-[#0d1170] text-white px-6 py-2 rounded-full hover:shadow-md transition"
                onClick={handleResend}
                disabled={resending}
              >
